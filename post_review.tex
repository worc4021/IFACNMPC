...
outcome of the previous stage). Theoretical properties of these controllers are well 
understood \citep[e.g.][]{Mayne:2000}. However, the numerical computation of robust model predictive 
controllers still poses
...
In almost all existing methods for RMPC, the model disturbances are assumed to be confined to sets that are constant, 
i.e.~independent of the system state and control inputs. Disturbances that depend on model states (or control inputs) 
are considered in~\cite{Kuntsevich:1995,Rakovic:2006}. However general formulations of state and input dependence can 
result in non-convex constraints~\citep{Rakovic:2006}. To avoid this we make use of recent algorithmic 
advances~\citep{Schaich:2015} that enable a class of polytopic disturbance sets that depend affinely on the system 
state and control input to be handled using convex constraints. This provides an algorithmic and analytical framework 
...
% \edit{In all aforementioned work and, to the authors' best knowledge, in all of the published literature on algorithmic
...
\end{equation}
%
where each element of $H(x,u)$ is a convex, piecewise affine function for all $(x,u)\in\mathcal X\times\mathcal U$. 
%\edit{where $H(x,u)$ is an elementwise convex, piecewise affine function}.
...
contained in $\mathcal{X}^\infty$. We define $\mathcal{X}^\infty$ as the maximal robust positive invariant 
(MRPI) set \citep{blanchini:2007}), for the closed-loop system
governed by a given
%stabilising\NOTE{Really?! We don't really need it to be stabilising for this definition} 
...
to be formulated in a convenient way. The following result shows that the MRPI set (\ref{eq:definition:MRPI:set}) 
...
%
\begin{thm}[\cite{Schaich:2015}]\label{thm:MRPI:set:is:polytopic}
Let $\mathcal X \subseteq \{x:\Gamma x\leq{\bf{1}}\wedge-\Gamma x\leq{\bf{1}}\}$ be a polyhedral set, let the pair $(A+BK,\Gamma)$ be
observable, and let $\mathcal W(x,Kx)$ be defined as in~\eqref{eq:PWA:def:disturbance} with an elementwise convex piecewise
affine function $H(x,Kx)$ which is bounded for all finite $x$. Then the MRPI set $\mathcal X^\infty$ is polytopic, i.e.\ it has the representation 
...
The conditions of Theorem~\ref{thm:MRPI:set:is:polytopic} require that $\mathcal W(x,u)$ is polytopic
for all finite $(x,u)$, i.e.\ it must have a convex hull representation $\mathcal W(x,u) = \conv_k\{w_k(x,u)\}, k\in\{1,\dots,r\}$ where each 
...
According to Theorem~\ref{thm:MRPI:set:is:polytopic} the MRPI set $\mathcal X^\infty$ is polytopic and has the 
...
\]
%and can be made arbitrarily small by the choice of $\beta$. 
...
any purpose. For a general mpQP, the condition~\eqref{mpQP:compatability:assumption} ensures that~\eqref{mpQP:primal}
% is a restriction on the set
% of parameters $\theta$ for which~\eqref{mpQP:primal} 
has a solution of the form~\eqref{mpQP:solution}.
...
%the so called compatibility constraint. 
...
The previous section presented a recursive algorithm to solve~\eqref{eq:the:problem}
...
With~\eqref{eq:trajectory:affine:representation} we can justify choosing the degeneracy parameters as
$\beta_m=\zeta_{m+1}$ and $\hat\beta_m=\hat\zeta_m$. When a constraint is activated the associated dual variable starts at zero and depends affinely on $(x_N,\beta_N)$.
%changesin an affine way in the appropriate direction, 
When the active constraints of a sub-problem become linearly dependent three things happen: the dual variables to the degenerate constraints
depend on an additional degeneracy parameter, a compatibility constraint is introduced in the predecessor
problem, and that compatibility constraint's dual variable is given as a affine function starting at zero,
while all other sub-problems are not affected by the degeneracy. Through the proposed choice we couple
...
Given the known set of active
constraints at $\chi_0$ we perform a line search towards $\chi_e$ via  
...
This paper proposes a receding horizon controller for linearly constrained linear systems with unknown
disturbances subject to state and input dependent bounds. The optimisation of predicted feedback laws
...
a guarantee of closed loop stability.
