...
\edit{In almost all existing methods for RMPC, the model disturbances are assumed to be confined to sets that are constant, i.e.~independent of the system state and control inputs. Disturbances that depend on model states (or control inputs) are considered in~\cite{Kuntsevich:1995,Rakovic:2006}. However general formulations of state and input dependence can result in non-convex constraints~\citep{Rakovic:2006}. To avoid this we make use of recent algorithmic advances~\citep{Schaich:2015} that enable a class of polytopic disturbance sets that depend affinely on the system state and control input to be handled using convex constraints. This provides an algorithmic and analytical framework which is used in the
...
\end{equation}
%
\edit{where each element of $H(x,u)$ is a convex, piecewise affine function for all $(x,u)\in\mathcal X\times\mathcal U$}. 
...
%
\edit{\begin{thm}[\cite{Schaich:2015}]\label{thm:MRPI:set:is:polytopic}
...
any purpose. \edit{For a general mpQP, the condition~\eqref{mpQP:compatability:assumption} ensures that~\eqref{mpQP:primal}
...
\textcolor{red}{With~\eqref{eq:trajectory:affine:representation} we can justify \edit{choosing the degeneracy parameters as
